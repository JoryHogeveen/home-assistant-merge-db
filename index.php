				...
				success: function( resp ) {
					...
					if ( resp.hasOwnProperty( 'messages' ) && resp.messages ) {
						...
						$.each( resp.messages, function( key, value ) {
							...
							if ( value.hasOwnProperty( 'log' ) && value.log ) {
								if ( 'object' === typeof value.log ) {
									$.each( value.log, function( lkey, lvalue ) {
										if ( isNaN( lkey ) ) {
											log += lkey + ': ' + lvalue + ' <br/>';
										} else {
											log += lvalue + ' <br/>';
								...
								log = '<pre class="collapse" id="' + key + '">' + log + '</pre>';
								log = '<a href="#" class="badge badge-light my-1" id="toggle-' + key + '" data-target="#' + key + '">log</a>' + log;
